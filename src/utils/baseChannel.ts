...
export class BaseStateChannel {
    ...
    constructor(
        ...
        public readonly accountAddress: AztecAddress,
        public readonly accountPrivkey: GrumpkinScalar,
    ...
    public static signOpenChannel(
        ...
        const signature = signSchnorr(channelMsg, senderPrivKey);
    ...
    public clone(): BaseStateChannel {

        // Create new class instance
        const address = this.accountAddress;
        const privkey = this.accountPrivkey;
        const contractAddress = this.contractAddress;
        const gameIndex = this.gameIndex;
        const pxe = this.pxe;
        const copy = new BaseStateChannel(address, privkey, contractAddress, gameIndex, pxe);

        // Copy results
        copy.openChannelResult = this.openChannelResult;
        copy.turnResults = this.turnResults;
        copy.orchestratorResult = this.orchestratorResult;
    ...
    public async openChannel(
        ...
        this.openChannelResult = await this.pxe.simulateAppCircuit(
            packedArguments,
    ...
    public async turn(
        ...
        let turnCapsule = encapsulateTurn({
            move: move,
            signatures: {
                sender: move.sign(this.accountPrivkey),
                opponent: opponentSignature,
            },
            timeout: opponentSignature === undefined,
        });
        await this.pxe.addCapsule(turnCapsule);
        // get the packed arguments for the call
        let packedArguments = await contract.methods
            .turn(this.gameIndex)
            .create()
        ...
        const result = await this.pxe.simulateAppCircuit(
            packedArguments,
    ...
    public async orchestrate() {
        ...
        this.orchestratorResult = await this.pxe.simulateAppCircuit(
            packedArguments,
    ...
    public async finalize(): Promise<TxReceipt> {
        ...
        let request = await contract.methods
            ...
            .create();
        let tx = await this.pxe.proveSimulatedAppCircuits(
            request,
            this.orchestratorResult!
        );
        // broadcast the transaction
        let result = await new SentTx(this.pxe, this.pxe.sendTx(tx)).wait({ debug: true });
        if (result.status !== TxStatus.MINED)
            throw new Error(`State channel finalization status is ${result.status}`);
        return this.pxe.getTxReceipt(result.txHash);
    }
    ...
    public async getContract(): Promise<TicTacToeContract> {
        const wallet = await getSchnorrWallet(this.pxe, this.accountAddress, this.accountPrivkey);
        return await TicTacToeContract.at(this.contractAddress, wallet);
    }
}
